...
function getCrypto(): { subtle: SubtleCrypto; getRandomValues: (array: Uint8Array) => Uint8Array } {
  ...
  const crypto = window.crypto || (globalThis as any).crypto || (self as any).crypto;
  ...
  if (!crypto.subtle) {
    throw new Error('Web Crypto API (crypto.subtle) is not available. This may be due to:\n' +
