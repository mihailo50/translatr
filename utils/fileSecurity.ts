 ...
 */
export async function stripImageMetadata(file: File, quality: number = 0.92): Promise<File> {
...
/**
 * Uploads a file in chunks for better performance and progress tracking
 */
export async function uploadFileInChunks(
  file: File,
  uploadFn: (chunk: Blob, chunkIndex: number, totalChunks: number) => Promise<void>,
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    uploadPromises.push(uploadFn(chunk, i, totalChunks));
  }
