...
export const useUserStatus = (user: { id: string } | null | undefined) => {
  const [status, setStatus] = useState<UserStatus>("online");
  const [onlineUsers, setOnlineUsers] = useState<Record<string, UserStatus>>({});
  const [_lastSeenMap, setLastSeenMap] = useState<Record<string, number>>({}); // Track last seen timestamps (for future use)
  ...
  const trackPresence = useCallback(
    async (uid: string, currentStatus: UserStatus, channel: RealtimeChannel) => {
  ...
  useEffect(() => {
    ...
    channel
      .on("presence", { event: "sync" }, () => {
        ...
        for (const _key in newState) {
          const presence = newState[_key][0] as
            | { user_id?: string; status?: UserStatus; last_seen?: string }
            | undefined;
          if (presence && presence.status && presence.user_id) {
      ...
      .on("presence", { event: "join" }, ({ key: _key, newPresences }) => {
        ...
        (
          newPresences as Array<{ user_id?: string; status?: UserStatus; last_seen?: string }>
        ).forEach((presence) => {
          if (presence.status && presence.user_id) {
            userMap[presence.user_id] = presence.status;
            // Track last seen timestamp
            if (presence.last_seen) {
              seenMap[presence.user_id] = new Date(presence.last_seen).getTime();
    ...
    channel.on("presence" as any, { event: "update" }, ({ key: _key, newPresences }) => {
    ...
    channel
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .on("presence" as any, { event: "leave" }, ({ key: _key, leftPresences }) => {
        const userMap: Record<string, UserStatus> = {};
        (leftPresences as Array<{ user_id?: string; status?: UserStatus }>).forEach((presence) => {
          if (presence.user_id) {
          ...
          Object.keys(userMap).forEach((userId) => {
            ...
            void (async () => {
              try {
                ...
                // Failed to update offline status - silently ignore
              }
            })();
          });
          return updated;
            ...
            if (
              ...
              void (async () => {
                try {
                  ...
                  // Failed to update user status to offline - silently ignore
                }
              })();
            }
          });
    ...
    const handleBeforeUnload = () => {
      if (user?.id) {
        ...
        void (async () => {
          try {
            await supabase.from("profiles").update({ status: persisted }).eq("id", user.id);
          } catch (_err) {
    ...
    const hiddenTimeoutRef: { current: NodeJS.Timeout | null } = { current: null };
          ...
          if (user?.id && document.hidden && preferredStatusRef.current !== "invisible") {
            ...
            void (async () => {
              try {
                ...
                // Failed to update status to offline - silently ignore
              }
            })();
          }
        }, 300000); // 5 minutes
        ...
        if (user?.id && status === "offline") {
          ...
          void (async () => {
            try {
              ...
              // Failed to restore status - silently ignore
            }
          })();
        }
      }
  ...
  useEffect(() => {
    ...
    const profilesChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          const updatedUser = payload.new as { id?: string; status?: string } | null;
          const oldUser = payload.old as { id?: string; status?: string } | null;
          ...
          if (
            ...
            updatedUser.id &&
            updatedUser.status &&
            updatedUser.status !== oldUser?.status
          ) {
  ...
  const updateUserStatus = useCallback(
    async (newStatus: UserStatus) => {
      ...
      const updateStatusPromise = supabase
        ...
        .eq("id", user.id);
      Promise.resolve(updateStatusPromise).catch((_error: unknown) => {
        // Failed to update status in database - silently ignore
      });
    },
    [mapToPersistedStatus, supabase, user, trackPresence]
  );
