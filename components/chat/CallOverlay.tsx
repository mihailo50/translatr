      ...
      <div className="absolute bottom-3 left-3 bg-black/60 px-2 py-1 rounded text-xs text-white/90 font-light tracking-wide flex items-center gap-2">
        ...
        {isSpeaking && !isScreenShare && (
          <span className="flex gap-0.5">
            ...
            />
            <span
              className="w-1 h-2 bg-indigo-400 rounded-full animate-pulse"
...
const FloatingLocalVideo = () => {
  ...
  const handleMove = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const newPos = clampPosition(clientX - offsetRef.current.x, clientY - offsetRef.current.y);
      setPosition(newPos);
    },
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute bottom-1 left-2 text-[10px] font-light tracking-wide text-white/90 z-20 pointer-events-none">
        You
      </div>
    </div>
  );
...
const HiddenCallListener = ({
  ...
  useEffect(() => {
    ...
    const checkForCallParticipants = () => {
      const callParticipant = remoteParticipants.find(
        (p) => p.identity !== userId && hasMediaTracks(p)
      );
      if (callParticipant) {
    ...
    const handleTrackSubscribed = (
      ...
      if (
        ...
        (publication.kind === "audio" || publication.kind === "video")
      ) {
        notifyParticipantJoined();
      }
    };
...
const CallContent = ({
      ...
      if (remoteParticipants.length > 0 && localParticipant && !participantJoinedRef.current) {
        ...
        try {
          ...
          localParticipant.publishData(encodedPayload, { reliable: true }).catch(() => {
            ...
            localParticipant.publishData(encodedPayload, { reliable: false }).catch(() => {});
          });
  ...
  const _remoteScreenShareTracks = screenShareTracks.filter(
    (t) => t.participant.identity !== localParticipant.identity
  );
    ...
    try {
      if (isScreenShareEnabled) {
        ...
        try {
          ...
          const errorObj = error instanceof Error ? error : { name: "", message: "" };
          if (errorObj.name === "NotAllowedError" || errorObj.message?.includes("permission")) {
            ...
            // User cancelled the screen share dialog - silently handle
          } else {
            toast.error("Failed to share screen", {
              description: errorObj.message || "Please check your permissions and try again.",
            });
          }
        }
      }
  ...
  // Call timer effect - use ref to track previous room state to avoid setState in effect
  ...
  useEffect(() => {
    ...
    prevRoomStateRef.current = currentState || null;
  ...
  // Use ref to track previous screenShareTracks length to avoid setState in effect
  const prevScreenShareTracksLengthRef = useRef<number>(0);

  useEffect(() => {
    const currentLength = screenShareTracks.length;
    const prevLength = prevScreenShareTracksLengthRef.current;

    // Only update state when transitioning from tracks to no tracks (or vice versa)
    if (currentLength === 0 && prevLength > 0) {
      ...
      // When tracks appear, default to first one
      requestAnimationFrame(() => {
        if (!focusedTrackSid) {
    ...
    prevScreenShareTracksLengthRef.current = currentLength;

    // If no focused track and we have tracks, default to first one
    ...
    if (focusedTrackSid && screenShareTracks.length > 0) {
  ...
  useEffect(() => {
    ...
    const ensureMicrophoneEnabled = async () => {
      try {
        ...
        // Failed to enable microphone - silently handle
      }
    };
  ...
  useEffect(() => {
          ...
          try {
            ...
            setTimeout(async () => {
              try {
                ...
                // Final attempt failed - silently handle
              }
            }, 100);
          }
        };
    ...
    const audioElementsMap = audioRefs.current;

    return () => {
      // Cleanup on unmount
      room.off(RoomEvent.Connected, handleConnected);
      audioElementsMap.forEach((audioElement) => {
        audioElement.pause();
        audioElement.srcObject = null;
        audioElement.remove();
      });
  ...
  useEffect(() => {
    ...
    const checkConnectionState = () => {
      ...
      if (room.state === "disconnected") {
        ...
        const lastError = (room as { lastError?: Error }).lastError ?? undefined;
  ...
  return (
    <div className="relative h-dvh w-full flex flex-col bg-[#020205]">
      ...
      <div className="fixed top-20 md:top-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md z-[100] w-auto max-w-[90vw]">
        ...
        <span className="text-[10px] tracking-[0.2em] font-bold text-emerald-400/80 uppercase">
          SECURE E2EE
        </span>
        ...
        {(() => {
          ...
          if (hasScreenShare) {
            ...
            const allCameraTracks = [
              ...remoteVideoTracks,
              ...(!isVideoOff && localVideoTrack ? [localVideoTrack] : []),
            ];
                ...
                {screenShareTracks.length > 1 && (
                  <div className="absolute top-24 left-1/2 -translate-x-1/2 z-50 flex gap-2 bg-black/50 backdrop-blur-md p-1.5 rounded-full border border-white/10">
                    {screenShareTracks.map((t) => {
                      const isActive =
                        activeScreenTrack.publication.trackSid === t.publication.trackSid;
                ...
                {allCameraTracks.length > 0 && (
                  <div className="h-32 w-full bg-[#0B0D12] border-t border-white/10 p-4 flex gap-3 overflow-x-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent snap-x">
                    {allCameraTracks.map((track) => (
                      <div
                        key={`camera-${track.participant.identity}`}
                        className="h-full aspect-video flex-shrink-0 snap-center"
                      >
          ...
          if (hasRemoteVideo) {
            ...
            return (
              <div
                className={`grid gap-4 w-full h-full ${remoteVideoTracks.length === 1 ? "grid-cols-1" : "grid-cols-2 md:grid-cols-3"}`}
              >
              ...
              {remoteParticipants.length === 0 ? (
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-8 flex-wrap">
                  {remoteParticipants.map((participant) => {
                    // Audio track is handled by audioRefs in useEffect
      ...
      <div
        className="fixed bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 w-fit px-6 py-3 rounded-full bg-white/[0.03] border border-white/10 backdrop-blur-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center gap-4 md:gap-8 z-[100]"
      ...
      >
        ...
        <button
          ...
          className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center bg-white/5 border border-white/10 transition-all hover:scale-110 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] min-w-[44px] min-h-[44px] ${isMuted ? "bg-red-500/20 border-red-500/30" : ""}`}
        >
          {isMuted ? (
        ...
        <button
          ...
          className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center bg-white/5 border border-white/10 transition-all hover:scale-110 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] min-w-[44px] min-h-[44px] ${isVideoOff ? "bg-red-500/20 border-red-500/30" : ""}`}
        >
          {isVideoOff ? (
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-all hover:scale-110 min-w-[44px] min-h-[44px] ${
...
const CallOverlay: React.FC<CallOverlayProps> = ({
  ...
  callType,
  onDisconnect,
  userPreferredLanguage: _userPreferredLanguage = "en",
  userId,
  onParticipantJoined,
  onCallAccepted,
  hidden = false,
      ...
      <div className="sr-only" aria-hidden="true">
        <LiveKitRoom
          ...
          onError={(error) => {
            ...
            // Silently handle unexpected errors for hidden listener
          }}
          options={{
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-[#020205] text-white animate-in fade-in duration-300 h-dvh">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute bottom-[-10%] right-[-10%] w-[70%] h-[70%] bg-purple-600/10 blur-[120px] rounded-full animate-nebula"
