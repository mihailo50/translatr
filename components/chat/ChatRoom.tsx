...
// These components are only needed when a call is active, so they shouldn't block initial render
...
// Lazy-load call-related modals - only needed when there's an active call
const LiveKitCallModal = dynamic(() => import("./LiveKitCallModal"), {
  ssr: false,
  loading: () => null, // Modal handles its own loading state
});

const CallNotificationBanner = dynamic(() => import("./CallNotificationBanner"), {
  ssr: false,
  loading: () => null, // Banner handles its own loading state
});
...
import {
  createCallRecord,
  updateCallRecord,
  updateCallRecordByCallId,
  getCallRecords,
} from "../../actions/callRecords";
...
const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  useEffect(() => {
    ...
    const checkForCallNotifications = async () => {
      ...
      try {
        ...
        if (callsError) {
          // Error fetching call notifications - silently handle
        }
        ...
        if (calls && calls.length > 0) {
          ...
          const content = latestCall.content as {
            sender_name?: string;
            call_type?: string;
            call_id?: string;
          };
          ...
          setCallType(callTypeFromNotif as "audio" | "video");
          ...
          callLogger.callUIShown({
            ...
            callType: callTypeFromNotif as "audio" | "video",
      ...
      .on(
        ...
        async (payload) => {
          const notification = payload.new as {
            ...
            related_id?: string;
            id?: string;
            content?: { sender_name?: string; call_type?: string; call_id?: string };
          };
          ...
          if (
            ...
            isCallModalOpenRef.current ||
            showCallBannerRef.current
          ) {
            return;
          }

          // Skip if already processed
          if (notification.id && processedCallNotifIdsRef.current.has(notification.id)) {
            return;
          }
          ...
          const content = notification.content as
            | { sender_name?: string; call_type?: string; call_id?: string }
            | undefined;
          const callerId = content?.sender_name || "Unknown";
          const callTypeFromNotif = content?.call_type || "audio";
          const callId = content?.call_id;
          ...
          setIncomingCallNotifId(notification.id || null);
        }
      )
      .subscribe();
  ...
  }, [messages]); // Depend on messages array to properly track changes
    ...
    if (lastMessage && !lastMessage.isMe && lastMessage.id !== lastMessageIdRef.current) {
      ...
      const playSound = () => {
        if (messageSoundRef.current) {
          ...
          messageSoundRef.current.play().catch((_err: unknown) => {
  ...
  }, [incomingCallNotifId, supabase, roomId]);
  ...
  useEffect(() => {
    ...
    const _callId = searchParams?.get("callId");
  ...
  const directPartnerId =
    roomDetails.room_type === "direct" ? roomDetails.participants?.[0]?.id : undefined;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const updatedProfile = payload.new as { status?: UserStatus };
          if (updatedProfile.status) {
            setPartnerProfileStatus(updatedProfile.status);
          }
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    ...
    const fetchGroupStatuses = async () => {
      ...
      const { data } = await supabase.from("profiles").select("id, status").in("id", memberIds);
        ...
        (payload) => {
          ...
          if (updated.id && memberIds.includes(updated.id)) {
            ...
            setGroupMemberDbStatuses((prev) => ({
              ...prev,
              [updatedId]: updated.status || "online",
            }));
          }
        }
      )
  ...
  useEffect(() => {
    ...
    if (isMenuOpen || isGroupMembersOpen)
  ...
  useEffect(() => {
    ...
    const handleData = async (payload: Uint8Array, _participant?: unknown) => {
  ...
  const handleStartCall = async (type: "audio" | "video") => {
    ...
    if (roomDetails.room_type === "direct") {
      ...
      calleeName = memberCount > 0 ? `Group (${memberCount} members)` : "Group Call";
    }
      ...
      callTimeoutRef.current = setTimeout(async () => {
        ...
        if (
          ...
          try {
            ...
            await liveKitChatRoom.localParticipant.publishData(encoder.encode(payload), {
              reliable: true,
            });
      ...
      const waitForConnectionAndSend = async () => {
        ...
        try {
          ...
          if (liveKitChatRoom.state !== ConnectionState.Connected) {
            ...
            const hasCSPError =
              ...
              (document.querySelector("[data-csp-violation]") !== null ||
                ...
                (window as { __CSP_VIOLATION__?: boolean }).__CSP_VIOLATION__);

            if (hasCSPError) {
              toast.error(
                "Connection error - possible security policy violation. Please check browser console."
              );
            }
            return;
          }
          ...
          if (!liveKitChatRoom.localParticipant) {
            toast.error("Failed to initialize call - connection not ready");
            return;
          }
          ...
          const safeSendData = async (packet: Uint8Array): Promise<boolean> => {
            try {
              ...
              if (
                errorMessage.includes("PC manager") ||
          ...
          // Send primary invite
          await safeSendData(encoder.encode(dataPacket));

          // Send backup invites to ensure delivery (with safe send)
          setTimeout(() => {
            safeSendData(encoder.encode(dataPacket)).catch(() => {});
          }, 500);
  ...
  const handleEndCall = async (_shouldSignalTerminate: boolean) => {
    ...
    if (liveKitChatRoom) {
      try {
        ...
        if (liveKitChatRoom.state !== ConnectionState.Connected) {
          // Room is not connected, skip call end signal
        } else if (!liveKitChatRoom.localParticipant) {
          // Local participant not available, skip call end signal
        } else {
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messages.filter((msg: { text?: string; translations?: Record<string, unknown> }) => {
      const textMatch = msg.text?.toLowerCase().includes(lowerQuery);
      const translationMatch =
        msg.translations &&
        Object.values(msg.translations).some(
          (t: unknown) => typeof t === "string" && t.toLowerCase().includes(lowerQuery)
        );
  ...
  const getStatusColor = (s: UserStatus) => {
    switch (s) {
      ...
      case "busy":
        return "bg-red-500";
      case "dnd":
        return "bg-red-500"; // Could include icon
      case "in-call":
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    {roomDetails.room_type === "group" ? (
                      ...
                      <div className="relative shrink-0">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-tr from-indigo-500/40 to-purple-500/40 p-[1px]">
                          <div className="w-full h-full rounded-full bg-[#03030b] flex items-center justify-center overflow-hidden">
                            {roomDetails.participants?.[0]?.avatar ? (
                              <Image
                                src={roomDetails.participants[0].avatar}
                                width={40}
                                height={40}
                        ...
                        {!isBlocked && (
                          <div
                            className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#03030b] ${getStatusColor(directPartnerStatus)}`}
                          ></div>
                        )}
                      </div>
                    )}
                        ...
                        <div className="relative group/shield shrink-0">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 rounded-lg bg-white/10 backdrop-blur-md border border-white/20 shadow-lg opacity-0 group-hover/shield:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50">
                            <p className="text-xs text-white/90 font-medium">
                              E2EE Secured by Translatr Protocol
                            </p>
                            {/* Tooltip Arrow */}
                            ...
                            {isGroupMembersOpen && (
                              <div className="absolute top-full left-0 mt-2 w-56 glass-strong rounded-xl border border-white/10 shadow-2xl overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-200">
                                <div className="px-3 py-2 border-b border-white/5 bg-white/5">
                                  <span className="text-[10px] font-bold uppercase text-white/50 tracking-wider">
                                    Online Members
                                  </span>
                                </div>
                                    ...
                                    onlineGroupMembers.map((m) => (
                                      ...
                                      >
                                        <div className="relative">
                                          ...
                                          <div
                                            className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-slate-900 ${getStatusColor(onlineUsers[m.id] as UserStatus)}`}
                                          ></div>
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <p className="text-sm text-white truncate">{m.name}</p>
                                          <p className="text-[10px] text-white/40 capitalize">
                                            {getStatusText(onlineUsers[m.id] as UserStatus)}
                                          </p>
                                        </div>
                                      </div>
                                    ))
                ...
                <div className="flex items-center gap-1 sm:gap-2 shrink-0">
                  ...
                  <div className="hidden md:flex items-center gap-1">
                    ...
                    >
                      <Languages
                        size={18}
                        className={isTranslationEnabled ? "text-aurora-indigo" : ""}
                      />
                    </button>
                    <button
                    ...
                    {roomDetails.room_type === "direct" && (
                      ...
                      >
                        {isBlocked && blockedByMe ? (
                          ...
                          <Ban
                            size={18}
                            className={isBlocked && !blockedByMe ? "text-white/30" : "text-red-400"}
                          />
                        )}
                      </button>
                    )}
                      ...
                      >
                        ...
                        >
                          <Languages
                            size={16}
                            className={
                              isTranslationEnabled ? "text-aurora-indigo" : "text-white/50"
                            }
                          />
                          <span className={isTranslationEnabled ? "text-white" : "text-white/70"}>
                            {isTranslationEnabled ? "Translation On" : "Translate Messages"}
                          </span>
                        </button>
                        <button
                          onClick={() => {
                            ...
                            {isBlocked && blockedByMe ? (
                              ...
                              <>
                                <Ban
                                  size={16}
                                  className={
                                    isBlocked && !blockedByMe ? "text-white/30" : "text-red-400"
                                  }
                                />
                                <span
                                  className={
                                ...
                                >
                                  {isBlocked && !blockedByMe ? "Blocked by User" : "Block User"}
                                </span>
                              </>
                            )}
                          </button>
