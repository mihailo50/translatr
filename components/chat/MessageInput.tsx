...
interface MessageInputProps {
  ...
  ) => Promise<{ encryptedText: string; iv: string; id?: string } | null>;
}
...
const MessageInput: React.FC<MessageInputProps> = ({
  ...
  }, []); // Generate once on mount - bars are animated via CSS, not regenerated
  ...
  const handleSend = async (e: React.FormEvent | React.MouseEvent) => {
    ...
    if (sendRealtimeMessage) {
      try {
        ...
        // Fast send failed, falling back to server - silently handle
      }
    }
    ...
    if (clientSentResult) {
      ...
      try {
        ...
        // Encryption failed - silently handle
      }
    }
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        const { file: processedFile, compressionRatio } = await processFileForUpload(originalFile);
        ...
        if (compressionRatio > 10) {
          toast.success(`File optimized: ${compressionRatio.toFixed(0)}% smaller`, {
            id: "file-processing",
          });
        } else {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              'File uploaded but not accessible. Please ensure the "attachments" bucket is public in Supabase Storage settings.'
            );
          }
          ...
          if (fetchError instanceof TypeError && fetchError.message === "Failed to fetch") {
            ...
            // Silently handle network errors during file accessibility check
          } else {
            // Other errors (file not accessible, etc.) - handle
            // Delete the uploaded file since it's not accessible
            await supabase.storage.from("attachments").remove([fileName]);
            throw new Error(
              "File uploaded but not accessible. The storage bucket may not be public."
            );
          }
        }
      ...
      {attachment && (
        <div className="pointer-events-auto self-start mb-2 ml-2 glass p-2 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 border border-white/10 shadow-lg relative z-[60]">
          ...
          <button
            ...
            className="p-1 hover:bg-white/10 rounded-full text-white/50 hover:text-white min-w-[44px] min-h-[44px] flex items-center justify-center"
          >
            <X size={14} />
          </button>
        </div>
          ...
          <>
            <div className="relative flex-1 flex items-center gap-3 px-4">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-white/10 text-white/70 hover:text-white transition-colors min-w-[44px] min-h-[44px]"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <button
              ...
              className="w-10 h-10 rounded-full flex items-center justify-center bg-red-500 text-white hover:bg-red-600 hover:scale-105 shadow-[0_0_20px_rgba(239,68,68,0.4)] transition-all duration-300 shrink-0 min-w-[44px] min-h-[44px]"
            >
          ...
          <>
            ...
            >
              {isSending ? (
                ...
                <Send size={18} fill="currentColor" className={isActive ? "ml-0.5" : ""} />
              )}
            </button>
          </>
        )}
