...
const CallRecordBubble: React.FC<CallRecordBubbleProps> = ({
    ...
    if (roomName && roomName !== "Loading..." && roomName !== "Unknown") {
      ...
      if (otherParticipants.length === 1) {
        ...
        const names = otherParticipants
          ...
          .join(", ");
        otherPersonName =
          otherParticipants.length > 2 ? `${names} +${otherParticipants.length - 2}` : names;
      } else {
  ...
  return (
    <div className="flex justify-center my-4">
      <div
        className={`flex items-center gap-2 px-4 py-2 rounded-full ${statusInfo.bgColor} ${statusInfo.borderColor} border text-sm ${statusInfo.color}`}
      >
