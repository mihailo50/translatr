...
import { Globe, RefreshCw, FileText, Download, Timer, X, Mic, Play, Pause } from "lucide-react";
    ...
    >
      <div className="absolute top-4 right-4 z-10">
        <button
          ...
          className="p-2.5 rounded-full bg-white/10 text-white hover:bg-white/20 transition-colors shadow-lg min-w-[44px] min-h-[44px] flex items-center justify-center"
        >
          <X size={24} />
        </button>
      </div>
  ...
  return createPortal(
    ...
    >
      <div className="absolute top-6 right-6">
        <button
          onClick={onClose}
          aria-label="Close view once image"
...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const shouldShowTranslation =
    isTranslationEnabled && !isOriginalLang && hasTranslation && !showOriginal;

  const displayText =
    shouldShowTranslation && message.translations?.[userPreferredLanguage]
  ...
  return (
    <div
      className={`flex items-end gap-3 ${message.isMe ? "flex-row-reverse" : "flex-row"} animate-in zoom-in-95 slide-in-from-bottom-2 duration-300`}
    >
      ...
      <div
        className={`group relative max-w-[75%] sm:max-w-[65%] flex flex-col ${message.isMe ? "items-end" : "items-start"}`}
      >
        {/* Bubble Container */}
        <div
          className={`
                      ...
                      >
                        {isViewed ? (
                          ...
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <span className="text-[10px] text-aurora-pink uppercase tracking-wider font-bold">
                              1 View Only
                            </span>
                          </div>
                        )}
