...
export async function createGroupRoom(name: string, memberIds: string[]): Promise<CreateGroupResult> {
  ...
  try {
    ...
    if (insertError) {
      console.error('Error creating group room members:', insertError);
      return { 
        success: false, 
