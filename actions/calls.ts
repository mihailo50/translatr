...
export async function checkActiveCall(
  roomId: string
): Promise<{ hasActiveCall: boolean; callType?: "audio" | "video"; participantCount?: number }> {
  try {
    const apiKey = process.env.LIVEKIT_API_KEY;
    const apiSecret = process.env.LIVEKIT_API_SECRET;
    const wsUrl = process.env.NEXT_PUBLIC_LIVEKIT_URL;
    ...
    try {
      ...
      if (rooms && rooms.length > 0) {
        ...
        if (room.numParticipants && room.numParticipants > 0) {
          ...
          const hasAudioTracks = participants.some(
            (p) => p.tracks?.some((t) => t.type === 0) // AUDIO track type
          );
          const hasVideoTracks = participants.some(
            (p) => p.tracks?.some((t) => t.type === 1) // VIDEO track type
          );
...
export async function initiateCall(
  ...
  try {
    ...
    const token = jwtResult && typeof jwtResult.then === "function" ? await jwtResult : jwtResult;
    ...
    try {
      ...
      if (supabaseUrl && supabaseUrl !== "https://placeholder.supabase.co") {
        ...
        if (recipientIds.length > 0) {
          ...
          await supabaseDb.from("notifications").insert(notifications);
        }
      }
    } catch (_notifError) {
